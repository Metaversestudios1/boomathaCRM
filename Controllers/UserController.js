...
const insertuser = async (req, res) => {
    const { password, role,managerId,...userData } = req.body;
    try {
      ...
      if (password.length < 4) {
       ...
       // Validate role (e.g., only allow specific roles like "Admin", "Manager", "Agent")
    ...
    if (role === "Agent" && !managerId) {
      return res.status(400).json({
        success: false,
        message: "managerId is required for Agents",
      });
    }
      ...
      const newUser = new User({
        ...
        ...(managerId && { managerId }), // Add manager_id if provided
